...
public class AdhocEvaluation extends Evaluation {
	...
	*/
     ...
     * @param QrelsFilename
	 */
	public void evaluate(ArrayList<String> resultFilename, String QrelsFilename) {
		//logger.info("Evaluating result file: "+resultFilename);
                qrels = new TRECQrelsInMemory(QrelsFilename);
		int effQueryCounter = 0;
		
		int[] numberOfRelevantRetrieved;
		int[] numberOfRelevant = null;
		int[] numberOfRetrieved;
		...
		try {
			...
			for (String str : resultFilename) {
				String [] line = str.split("\t");
				String queryid = line[0].trim();
				
				if (!qrels.queryExistInQrels(queryid))
					continue;

                                
				String docID = line[1].trim();
				 
				int rank = Integer.parseInt(line[2].trim());
			...
			for (int i = 0; i < effQueryCounter; i++){
				numberOfRelevantRetrieved[i] = 
					vecNumberOfRelevantRetrieved.get(i).intValue();
				numberOfRelevant[i] = vecNumberOfRelevant.get(i).intValue();
				numberOfRetrieved[i] = vecNumberOfRetrieved.get(i).intValue();
		...
		for (int i = 0; i < effQueryCounter; i++) {
			Record[] relevantRetrieved = listOfRelevantRetrieved.get(i);
	...
	public void writeEvaluationResultOfEachQuery(String resultEvalFilename) {
		try {
			...
			for (int i = 0; i < this.queryNo.length; i++)
				sb.append(queryNo[i]).append(" ").append(Rounding.toString(
	...
	public void writeEvaluationResult(PrintWriter out) {
        ...
        public String EvaluationResult(){
            return Rounding.toString(AveragePrecision, 4);
        }
